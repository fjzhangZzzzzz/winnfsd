...
- **#101 NFSv3 READ data length 错误**  
  - **现象**：在 Linux 5.19+ 等新内核上，通过 NFSv3 挂载后 `cat` 小文件会报 `Input/output error`，日志中可见 `READ count doesn't match length of opaque`。  
  - **原因**：READ 响应中，返回的 `count` 使用的是实际读取字节数，但 `opaque` 的 length 字段仍然是请求的大小（例如 4096），导致 NFS 客户端校验失败。  
  - **修复方式**：在 `CNFS3Prog::ProcedureREAD` 中，`fread` 返回后使用实际读取的字节数再次调用 `data.SetSize(count)`，保证 `opaque` 长度与 `count` 一致。
